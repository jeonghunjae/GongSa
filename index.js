...
app.get('/', async (req, res) => { 
  // 한국 시간으로 오늘의 날짜를 가져옵니다.
  const today = new Date().toLocaleDateString('en-CA'); // 'en-CA' 형식으로 날짜를 'YYYY-MM-DD'로 반환
...
app.get('/ManpowerFinalPaper', async (req, res) => {
    try {
      const selectedDate = req.query.date || new Date().toLocaleDateString('en-CA');
