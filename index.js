...
app.get('/manage', async (req, res) => {
  ...
  const writtenCompanies = await db.sql`
    SELECT wd.*, c.company AS companyName, c.trade, e.equipmentName, e.specification, we.equipmentCount
  ...
  // 작성되지 않은 업체 목록 구하기
  const notWrittenCompanies = await db.sql`
  SELECT *
  FROM companies
  WHERE id NOT IN (${writtenCompanyIds.join(', ')})
